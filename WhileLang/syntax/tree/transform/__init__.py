...
class TreeTransform:
    ...
    def __init__(
        self, transformers=[], dir=TOP_DOWN, recurse=False
    ):  # @ReservedAssignment
    ...
    def __call__(self, tree):
        ...
        def descend(root, subtrees):
            return type(tree)(root, self.flatten([self(s) for s in subtrees]))
    ...
    def _reapply(self, tree, last_transformer, inplace=False):
        if self.IS_DESCENDING or self.recurse:
            rerun = lambda x, tree, descent=True: (
                x.inplace(tree, descent=descent) if inplace else x(tree)
            )
            if self.recurse:
