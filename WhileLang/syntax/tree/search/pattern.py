...
class TreeRootPattern(TreePattern):
    ...
    def match(self, tree):
        ...
        if r == self.symbol and (self.fan is None or self.fan == len(s)):
            ...
            children = dict((numeral(i), s[i]) for i in range(len(s)))
...
class TreeTopPattern(TreePattern):
    ...
    def _rematch_subtrees(self, pattern, text, acc):
        ...
        ts = text.subtrees
        ellipsis = [
            i for i in range(len(ps)) if self._is_subtrees_placeholder(ps[i].root)
        ]
        if not ellipsis and len(ps) == len(ts):
            ...
            if len(ellipsis) > 1:
                raise NotImplementedError("more than one ellipsis child")
            for i in range(ellipsis[0]):
                mo = self._match(ps[i], ts[i])
                if mo is None:
                    return None
                acc.update(mo)
            for i in range(len(ps) - ellipsis[0] - 1):
                mo = self._match(ps[-1 - i], ts[-1 - i])
                if mo is None:
                    return None
                acc.update(mo)
            acc[ps[ellipsis[0]].root] = ts[
                ellipsis[0] : ellipsis[0] + len(ts) - len(ps) + 1
            ]
            return acc
        else:
            return None
    ...
    def _is_subtrees_placeholder(self, node_value):
        return (
            self._is_text(node_value)
            and node_value.startswith("$")
            and node_value.endswith("...")
        )
